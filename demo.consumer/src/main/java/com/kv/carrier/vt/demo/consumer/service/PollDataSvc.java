...
@Log4j2
@Service
public class PollDataSvc implements InitializingBean {
    ...
    public PollDataSvc(@Value("${demo.url}") String url,
    ...
    public RespDataDO apply(String ticker) {
        try (var scope = new StructuredTaskScope.ShutdownOnFailure()) {
            Subtask<String> random1 = scope.fork(() -> fetchDataFromTarget(Endpoints.RANDOM_ONE));
            Subtask<String> random2 = scope.fork(() -> fetchDataFromTarget(Endpoints.RANDOM_TWO));
            Subtask<String> random3 = scope.fork(() -> fetchDataFromTarget(Endpoints.RANDOM_THREE));
            Subtask<String> random4 = scope.fork(() -> fetchDataFromTarget(Endpoints.RANDOM_FOUR));
            Subtask<String> random5 = scope.fork(() -> fetchDataFromTarget(Endpoints.RANDOM_FIVE));
            Subtask<String> random6 = scope.fork(
                    () -> ScopedValue.callWhere(HEADER_KEY, key, () -> getTickerDetailsFromYahoo(ticker)));
            try {
                scope.join();
            ...
            return new RespDataDO(random1.get(), random2.get(), random3.get(), random4.get(), random5.get(), random6.get());
        }
    }

    private String fetchDataFromTarget(Endpoints task) {
        return client.get().uri(Endpoints.LOOKUP.get(task)).retrieve().toEntity(String.class).getBody();
    }

    private String getTickerDetailsFromYahoo(String ticker) {
